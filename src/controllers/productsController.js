...
export async function getProducts(req, res) {
    ...
    try {
        ...
        if(busca){
            ...
            products = await productsCollection.find({$text : {$search: busca}}).toArray(); 
            if(tipo){
...
export async function editProduct(req, res) {
    ...
    try {
        await db.collection("products").updateOne({ _id: new ObjectId(id) }, { $set: { titulo, descricao, valor, categoria} });
        res.sendStatus(200);
    }
    catch (err) {
        console.log(err);
